...
$(document.body).trigger('wc_cart_button_updated',[$button])}};AddToCartHandler.prototype.updateCartPage=function(){var page=window.location.toString().replace('add-to-cart','added-to-cart');$('.shop_table.cart').load(page+' .shop_table.cart:eq(0) > *',function(){$('.shop_table.cart').stop(!0).css('opacity','1').unblock();$(document.body).trigger('cart_page_refreshed')});$('.cart_totals').load(page+' .cart_totals:eq(0) > *',function(){$('.cart_totals').stop(!0).css('opacity','1').unblock();$(document.body).trigger('cart_totals_refreshed')})};AddToCartHandler.prototype.updateFragments=function(e,fragments){if(fragments){$.each(fragments,function(key){$(key).addClass('updating').fadeTo('400','0.6').block({message:null,overlayCSS:{opacity:0.6}})});$.each(fragments,function(key,value){$(key).replaceWith(value);$(key).stop(!0).css('opacity','1').unblock()});$(document.body).trigger('wc_fragments_loaded')}};new AddToCartHandler()});jQuery(function($){if(typeof wc_single_product_params==='undefined'){return!1}
