...
$(document.body).trigger('wc_cart_button_updated',[$button])}};AddToCartHandler.prototype.updateCartPage=function(){var page=window.location.toString().replace('add-to-cart','added-to-cart');$('.shop_table.cart').load(page+' .shop_table.cart:eq(0) > *',function(){$('.shop_table.cart').stop(!0).css('opacity','1').unblock();$(document.body).trigger('cart_page_refreshed')});$('.cart_totals').load(page+' .cart_totals:eq(0) > *',function(){$('.cart_totals').stop(!0).css('opacity','1').unblock();$(document.body).trigger('cart_totals_refreshed')})};AddToCartHandler.prototype.updateFragments=function(e,fragments){if(fragments){$.each(fragments,function(key){$(key).addClass('updating').fadeTo('400','0.6').block({message:null,overlayCSS:{opacity:0.6}})});$.each(fragments,function(key,value){$(key).replaceWith(value);$(key).stop(!0).css('opacity','1').unblock()});$(document.body).trigger('wc_fragments_loaded')}};new AddToCartHandler()});(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory)}else if(typeof module==='object'&&module.exports){module.exports=function(root,jQuery){if(jQuery===undefined){if(typeof window!=='undefined'){jQuery=require('jquery')}else{jQuery=require('jquery')(root)}}
...
return strength}};wc_password_strength_meter.init()})(jQuery);(function(factory){var registeredInModuleLoader=!1;if(typeof define==='function'&&define.amd){define(factory);registeredInModuleLoader=!0}
...
parent.removeClass('currentTarget')});$.scroll_to_notices=function(scrollElement){if(scrollElement.length){$('html, body').animate({scrollTop:(scrollElement.offset().top-100)},1000)}}});jQuery(function($){$('.lost_reset_password').on('submit',function(){$('button[type="submit"]',this).attr('disabled','disabled')})});jQuery(function($){if(typeof wc_cart_fragments_params==='undefined'){return!1}
