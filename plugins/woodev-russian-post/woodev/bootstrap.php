...
if ( ! class_exists( 'Woodev_Plugin_Bootstrap' ) ) :
	...
	class Woodev_Plugin_Bootstrap {
		 ...
		 */
		public function register_plugin( $framework_version, $plugin_name, $path, callable $callback, $args = array() ) {
		...
		public function load_plugins() {
			...
			usort( $this->registered_plugins, array( $this, 'framework_compare' ) );
		...
		public function render_deactivation_notice() {
			...
			echo $_GET['woodev_framework_deactivate_newer'] > 1 ?
				sprintf( _n( 'Deactivated %d plugin', 'Deactivated %d plugins', intval( $_GET['woodev_framework_deactivate_newer'] ), 'woodev-plugin-framework' ), $_GET['woodev_framework_deactivate_newer'] ) :
				__( 'Deactivated one plugin', 'woodev-plugin-framework' );
			echo '</p></div>';
		}
		 ...
		 */
		public function framework_compare( $a, $b ) {
