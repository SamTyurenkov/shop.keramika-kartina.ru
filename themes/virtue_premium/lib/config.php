...
function kadence_sidebar_on_search_page( $sidebar ) {
	...
	if( is_search() ) {
		if ( isset( $virtue_premium[ 'search_page_layout' ] ) && 'full' == $virtue_premium[ 'search_page_layout' ] ) {
			$sidebar = false;
		} else {
			$sidebar = true;
		}
...
function kadence_category_layout_css() {
	global $virtue_premium;
	if ( virtue_display_sidebar() ) {
		if ( isset( $virtue_premium['product_shop_layout'] ) && 'single' === $virtue_premium['product_shop_layout'] ) {
