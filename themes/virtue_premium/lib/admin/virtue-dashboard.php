...
function virtue_kadence_blocks_notice() {
	if ( get_transient( 'virtue_premium_blocks_plugin_notice' ) || ! current_user_can( 'manage_options' ) || ! function_exists( 'register_block_type' ) ) {
		return;
	}
	$installed_plugins = get_plugins();
...
function virtue_hide_blocks_plugin_notice() {
	if ( isset( $_GET['virtue-kadence-blocks-plugin-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
		...
		set_transient( 'virtue_premium_blocks_plugin_notice', 1, 4 * YEAR_IN_SECONDS );
	}
}
